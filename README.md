...
- **柔軟な動画長設定**: 1秒、6秒、8秒、10秒(5x2)、12秒(4x3)、16秒(4x4)、20秒(4x5)の7つのモードに対応　※独自機能（v1.5.1で1秒モード追加）
...
2. **セクション間の滑らかな接続**:
   ...
   - v1.5.1からキーフレーム自動コピー機能はデフォルトオフになりました。設定をオンにすれば細かく設定せずに済みますが、より詳細な制御が必要な場合はオフのままで個別設定可能です。
