...
- **柔軟な動画長設定**: 1秒、6秒、8秒、10秒、12秒、16秒、20秒の7つのモードに対応　※独自機能（v1.5.1で1秒モード追加、v1.6でカッコ表記を削除）
...
- **動画長設定**:　※既存機能の固定化
  ...
  ※v1.6では動画モード名のカッコ内説明（例：「10(5x2)秒」→「10秒」）が削除され、UIがシンプル化されました。また、フレーム数・セクション数も表示秒数と整合するように調整され、UI表示と実際の処理の一貫性が向上しています。
...
- **キーフレーム設定**:　※nirvash氏追加機能
  ...
  ※v1.6ではセクション設定UIがアコーディオンとして分離され、必要なときのみ展開できるようになりました。
...
1. **最終セクションの急激な変化を防止**:
   ...
   ※v1.6ではキーフレーム強調表示（赤枠）が削除され、シンプルなUIになりました。
...
4. **動的セクション計算の改善**:※v1.6で追加
   - 秒数から直接セクション数を計算する機能が追加され、一貫性と効率性が向上
   - フレーム数からセクション数を計算する代わりに、秒数から直接計算するロジックに変更
...
- **動画長**: ラジオボタン + `total_second_length` スライダー　※独自機能
  - **1秒**: 短い動画（約30フレーム @ 30fps）- v1.5.1で追加
  - **6秒**: 標準モード（約180フレーム @ 30fps）
  - **8秒**: 拡張モード（約240フレーム @ 30fps）
  - **10秒**: パート廃止（約300フレーム @ 30fps）
  - **12秒**: パート廃止（約360フレーム @ 30fps）
  - **16秒**: パート廃止（約480フレーム @ 30fps）
  - **20秒**: パート廃止（約600フレーム @ 30fps）
  
  ※v1.6では表示秒数と実際のフレーム数・セクション数の整合性が向上しています。
...
- **キーフレーム自動コピー**: `enable_keyframe_copy` チェックボックス（デフォルト: 無効 - v1.5.1で変更）　※独自機能
  ...
  - v1.6ではUI上から非表示になり、設定として内部化されました
