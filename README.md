...
- **柔軟な動画長設定**: 6秒、8秒、10秒(5x2)、12秒(4x3)の4つのモードに対応　※独自機能
- **プロンプト管理機能**: プロンプトの保存、編集、再利用が簡単　※独自機能
- **ログ機能**: 詳細な経過情報や終了時に処理時間を出したりWindowsだと音を出したりします(今のところ固定)　※独自機能
...
1. **最終セクションの急激な変化を防止**:
   - 元のFramePackでは、キーフレーム画像のヒット率が低く、最終セクション（最初の1秒付近）で急に画像が変わる問題がありました
   - FramePack-eichiはなら全部のセッションにキーフレームをぶち込んでやろうという安直な力業に出ています
   - 特に重要なキーフレームを赤枠で強調表示し、これらに画像を設定することで、自動コピーするようにしています
   - FramePackは上述の通り、最終セクションから動画を生成するため、セッションの並び及びキーフレーム場増も最後尾から設定する形になります
...
※ Linuxでの動作も基本的には可能ですが、一部機能に制限がある場合があります。
...
2. `endframe_ichi.py`を`webui`フォルダに配置します。
...
1. `endframe_ichi.py`をダウンロードして配置します。
...
### フレーム設定
- **動画長**: ラジオボタン + `total_second_length` スライダー　※新規機能
...
生成プロセス中に以下のようなエラーが発生することがあります：
```
h11._util.LocalProtocolError: Too little data for declared Content-Length
```

このエラーは、HTTPレスポンスの処理中に問題が発生した場合に表示されます。解決方法：

1. 画像を一度削除して、再度アップロードしてみてください。
2. 同じファイルで何度も失敗する場合：
