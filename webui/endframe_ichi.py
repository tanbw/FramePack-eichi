...
def worker(input_image, end_frame, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, save_section_frames, keep_section_videos, output_dir=None, section_settings=None, use_lora=False, lora_file=None, lora_scale=0.8, lora_format="HunyuanVideo", end_frame_strength=1.0, use_all_padding=False, all_padding_value=1.0):
    ...
    print(i18n.translate('[DEBUG] worker関数での秒数: {0}, latent_window_size: {1}, 計算されたセクション数: {2}').format(total_second_length, latent_window_size, total_latent_sections))
    ...
    try:
        ...
        def get_section_settings_map(section_settings):
            ...
            if section_settings is not None:
                # print(i18n.translate("\n[デバッグ] セクション設定処理開始: 総数={0}").format(len(section_settings)))
        ...
        def process_section_prompt(i_section, section_map, llama_vec, clip_l_pooler, llama_attention_mask):
            ...
            if section_map:
                ...
                if valid_section_nums:
                    ...
                    print(i18n.translate("[デバッグ] セクション{0}の処理: セクション{1}の設定を使用").format(i_section, section_num))
                else:
                    print(i18n.translate("[デバッグ] セクション{0}の処理: 有効なセクション設定が見つかりません").format(i_section))
            ...
            if section_info and len(section_info) > 1:
                ...
                if section_prompt and section_prompt.strip():
                    ...
                    try:
                        ...
                        print(i18n.translate("[デバッグ] セクション{0}用プロンプト処理完了").format(i_section))
        ...
        for i_section, latent_padding in enumerate(latent_paddings):
            ...
            if use_lora and has_lora_support and lora_file is not None:
                try:
                    ...
                    print(i18n.translate("LoRAを読み込み中: {0} (スケール: {1})").format(os.path.basename(lora_path), lora_scale))
            ...
            def callback(d):
                try:
                    ...
                    print(i18n.translate("[WARNING] プレビュー生成中にエラー: {0}").format(e))
                    current_step = d['i'] + 1
                    percentage = int(100.0 * current_step / steps)
                    hint = i18n.translate("Sampling {current_step}/{steps}").format(current_step=current_step, steps=steps)
                    section_info = i18n.translate("セクション: {0}/{1}").format(i_section+1, total_sections)
            ...
            if is_last_section:
                ...
                if not keep_section_videos:
                    ...
                    if deleted_count > 0:
                        ...
                        stream.output_queue.push(('progress', (None, i18n.translate("{0}個の中間ファイルを削除しました。最終動画は保存されています。").format(deleted_count), make_progress_bar_html(100, i18n.translate('処理完了')))))
...
def process(input_image, end_frame, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, use_random_seed, save_section_frames, keep_section_videos, output_dir, section_settings, use_lora=False, lora_file=None, lora_scale=0.8, lora_format="HunyuanVideo", end_frame_strength=1.0, use_all_padding=False, all_padding_value=1.0, frame_size_setting=None):
    global stream
    assert input_image is not None, i18n.translate('No input image!')
    ...
    if abs(total_second_length - expected_seconds) > 0.1:  # 小数点の誤差を考慮
        print(i18n.translate('[注意] 動画長設定の不一致を修正: UIモード「{0}」の期待値は{1}秒ですが、スライダー値は{2}秒でした').format(current_mode, expected_seconds, total_second_length))
    ...
    if use_lora and lora_file is not None:
        ...
        print(i18n.translate("\u25c6 LoRAフォーマット: {0}").format(lora_format))
...
with block:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            all_padding_value = gr.Slider(label=i18n.translate("パディング値"), minimum=0, maximum=3, value=1, step=1, info=i18n.translate("すべてのセクションに適用するパディング値（0〜3の整数）"), visible=False)
    ...
    with gr.Row():
        with gr.Column():
            input_image = gr.Image(sources='upload', type="numpy", label=i18n.translate("Image"), height=320)
            ...
            def update_section_title(frame_size, mode, length):
                ...
                # print(i18n.translate("[デバッグ] セクションタイトル更新: 元の値={0}, 表示用に調整した値={1}").format(total_sections, display_sections))
                ...
                with gr.Group(elem_classes="section-container"):
                    ...
                    for i_reverse in range(max_keyframes):
                        ...
                        with gr.Row(visible=(i < initial_sections_count), elem_classes="section-row") as row_group:
                            # 左側にセクション番号とプロンプトを配置
                            with gr.Column(scale=1):
                                section_number = gr.Number(label=i18n.translate("セクション番号"), value=i, precision=0)
                                section_prompt = gr.Textbox(label=i18n.translate("セクションプロンプト"), placeholder=i18n.translate("セクション固有のプロンプト（空白の場合は共通プロンプトを使用）"), lines=2)

                            # 右側にキーフレーム画像のみ配置
                            with gr.Column(scale=2):
                                # セクション番号に応じて表示内容を変更
                                section_image = gr.Image(label=i18n.translate("キーフレーム画像"), sources="upload", type="numpy", height=200)
            ...
            def collect_section_settings(*args):
                ...
                for i in range(0, len(args), 3):
                    if i+2 < len(args):  # 配列の範囲チェック
                        ...
                        if num is not None:  # Noneでないセクション番号のみ処理
                            ...
                            # print(i18n.translate("[デバッグ] セクション追加: セクション番号={0}, 画像有無={1}, プロンプト長={2}").format(num, img is not None, len(prompt) if prompt else 0))
                ...
                # print(i18n.translate("[デバッグ] ソート後のセクション番号順: {0}").format(section_nums))
        ...
        with gr.Column():
            ...
            preview_image = gr.Image(label=i18n.translate("Next Latents"), height=200, visible=False)
            ...
            with gr.Group(visible=has_lora_support) as lora_settings_group:
                gr.Markdown(f"### {i18n.translate('LoRA設定')}")
