...
def initialize_presets():
    ...
    for i, prompt_text in enumerate(default_prompts):
        presets_data["presets"].append({
            "name": i18n.translate("デフォルト {i}: {prompt_text}...").format(i=i+1, prompt_text=prompt_text[:20]),
            "prompt": prompt_text,
            "timestamp": datetime.now().isoformat(),
            "is_default": True
        })
        ...
        try:
            with open(preset_file, 'r', encoding='utf-8') as f:
                ...
                if not file_contents.strip():
                    print(i18n.translate("読み込み時に空ファイルが検出されました: {0}").format(preset_file))
                    # 空ファイルの場合は再初期化を試みる
                    initialize_presets()
                    retry_count += 1
                    continue
                ...
                print(i18n.translate("プリセットファイル読み込み成功: {0}件").format(len(data.get('presets', []))))
                return data

        except (json.JSONDecodeError, UnicodeDecodeError) as e:
            # JSONパースエラーの場合はファイルが破損している可能性がある
            print(i18n.translate("プリセットファイルの形式が不正です: {0}").format(e))
            # ファイルをバックアップ
            backup_file = f"{preset_file}.bak.{int(datetime.now().timestamp())}"
            try:
                import shutil
                shutil.copy2(preset_file, backup_file)
                print(i18n.translate("破損したファイルをバックアップしました: {0}").format(backup_file))
            except Exception as backup_error:
                print(i18n.translate("バックアップ作成エラー: {0}").format(backup_error))
            ...
            print(i18n.translate("プリセット読み込みエラー: {0}").format(e))
            # エラー発生
            retry_count += 1

    # 再試行しても失敗した場合は空のデータを返す
    print(i18n.translate("再試行しても読み込みに失敗しました。空のデータを返します。"))
    return {"presets": []}

def get_default_startup_prompt():
    """起動時に表示するデフォルトプロンプトを取得する関数"""
    print(i18n.translate("起動時デフォルトプロンプト読み込み開始"))
    ...
    for preset in presets_data["presets"]:
        if preset.get("is_startup_default", False):
            ...
            print(i18n.translate("起動時デフォルトプロンプトを読み込み: '{0}'... (長さ: {1}文字)").format(startup_prompt[:30], len(startup_prompt)))

            # 重複しているかチェック
            # 例えば「A character」が複数回出てくる場合は重複している可能性がある
            if "A character" in startup_prompt and startup_prompt.count("A character") > 1:
                print(i18n.translate("プロンプトに重複が見つかりました。最初のセンテンスのみを使用します。"))
                # 最初のセンテンスのみを使用
                sentences = startup_prompt.split(".")
                if len(sentences) > 0:
                    clean_prompt = sentences[0].strip() + "."
                    print(i18n.translate("正規化されたプロンプト: '{0}'").format(clean_prompt))
                    return clean_prompt
    ...
    if "default_startup_prompt" in presets_data:
        ...
        print(i18n.translate("デフォルト設定から読み込み: '{0}'... (長さ: {1}文字)").format(default_prompt[:30], len(default_prompt)))

        # 同様に重複チェック
        if "A character" in default_prompt and default_prompt.count("A character") > 1:
            print(i18n.translate("デフォルトプロンプトに重複が見つかりました。最初のセンテンスのみを使用します。"))
            sentences = default_prompt.split(".")
            if len(sentences) > 0:
                clean_prompt = sentences[0].strip() + "."
                print(i18n.translate("正規化されたデフォルトプロンプト: '{0}'").format(clean_prompt))
                return clean_prompt
    ...
    print(i18n.translate("プログラムのデフォルト値を使用: '{0}'").format(fallback_prompt))
...
def save_preset(name, prompt_text):
    ...
    if not name:
        ...
        if not startup_default_exists:
            ...
            print(i18n.translate("起動時デフォルトを新規作成: {0}").format(prompt_text[:50]))
        ...
        try:
            ...
            return i18n.translate("プリセット '起動時デフォルト' を保存しました")
        except Exception as e:
            print(i18n.translate("プリセット保存エラー: {0}").format(e))
            traceback.print_exc()
            return i18n.translate("保存エラー: {0}").format(e)
    ...
    try:
        ...
        # ファイル保存成功
        return i18n.translate("プリセット '{0}' を保存しました").format(name)
    except Exception as e:
        print(i18n.translate("プリセット保存エラー: {0}").format(e))
        # エラー発生
        return i18n.translate("保存エラー: {0}").format(e)
...
def delete_preset(preset_name):
    ...
    if not preset_name:
        return i18n.translate("プリセットを選択してください")
    ...
    if not target_preset:
        return i18n.translate("プリセット '{0}' が見つかりません").format(preset_name)

    # デフォルトプリセットは削除できない
    if target_preset.get("is_default", False):
        return i18n.translate("デフォルトプリセットは削除できません")
    ...
    try:
        ...
        return i18n.translate("プリセット '{0}' を削除しました").format(preset_name)
    except Exception as e:
        return i18n.translate("削除エラー: {0}").format(e)