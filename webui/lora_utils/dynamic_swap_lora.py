...
class DynamicSwapLoRAManager:
    ...
    def save_original_states(self, model: torch.nn.Module) -> None:
        ...
        for name, param_name, param, score, full_param_name in selected_matches:
            ...
            saved_params += 1
            logger.info(i18n.translate("パラメータ保存: {full_param_name} (スコア: {score})").format(full_param_name=full_param_name, score=score))

        logger.info(i18n.translate("元の状態を保存: {saved_params}パラメータ（最大{self.max_parameters}個に制限）").format(saved_params=saved_params, self=self))
    ...
    def apply_to_layer(self, layer_name: str, layer: torch.nn.Module) -> torch.nn.Module:
        ...
        try:
            ...
            if applied_count > 0:
                ...
                logger.debug(i18n.translate("レイヤー「{layer_name}」に{applied_count}個のパラメータを適用").format(layer_name=layer_name, applied_count=applied_count))
