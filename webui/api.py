...
# --- 配置 ---
# 【重要】确保这个地址是你的 FramePack-eichi Gradio 应用正在运行的地址
GRADIO_APP_URL = "http://127.0.0.1:7862/"
gradio_client = None
# --- 初始化 ---
app = FastAPI(
...
async def generate_video(
    ...
    alarm_on_completion: bool = Form(True),
):
    """
    ...
    try:
        ...
        job = gradio_client.predict(
            **params,
            api_name="/validate_and_process" 
        )

        print("Gradio API调用完成。")

        # Gradio返回的是一个元组，第一个元素是视频，第二个是预览图，...
        # 我们只需要第一个元素
        video_path = job[0]

        if not video_path:
            raise HTTPException(status_code=500, detail="视频生成失败，Gradio应用没有返回文件路径。")
        ...
        raise HTTPException(status_code=500, detail=f"视频生成过程中发生内部错误: {e}")
